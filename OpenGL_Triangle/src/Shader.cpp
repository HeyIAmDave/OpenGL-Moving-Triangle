...
void Shader::Init(const std::string & vsrc, const std::string & fsrc)
{
	...
	GLuint vertexShader = CreateShader(GL_VERTEX_SHADER, vsrc);
	GLuint fragmentShader = CreateShader(GL_FRAGMENT_SHADER, fsrc);
...
GLuint Shader::CreateShader(GLenum type, const std::string& src)
{
